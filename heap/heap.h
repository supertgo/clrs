...
public:
  int rootCnt;
  Heap();
  void updateHead(int p, int q) {
    if (roots[p] != empty && !atom(roots[p]))
      Head(roots[p]) = roots[q];
  }
  void updateTail(int p, int q) {
    if (roots[p] != empty && !atom(roots[p]))
...
Heap::Heap() : empty(-1), OK(1), head(0), tail(1), maxHeap(6),
maxRoot(50) {
...
void Heap::collect() {
  ...
  for (p = markDescendants; p != empty; p = markDescendants) {
    transfer(p, markDescendants);
    if (!atom(p) && !marked(Head(p))) {
      transfer(Heap(p), nonFreeCells, markDescendants);
      marked(Heap(p)) = true;
    }
...
void Heap::allocateAtom(int p, int val) {
  if (allocateAux(p) == OK) {
    atom(roots[p]) = true;
    value(roots[p]) = val;
  }
...
void Heap::allocateList(int p, int q, int r) {
  if (allocateAux(p) == OK) {
    atom(roots[p]) = false;
    Head(roots[p]) = roots[q];
